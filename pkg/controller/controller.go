...
func NewController(clientset *kubernetes.Clientset, kubeInformerFactory kubeinformers.SharedInformerFactory, stopCh <-chan struct{}) (*Controller, error) {
	...
	recorder := eventBroadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "gputopo-schd-extender"})
	...
	klog.Infoln("begin to wait for cache")
	...
	if ok := clientgocache.WaitForCacheSync(stopCh, c.nodeInformerSynced); !ok {
		...
		klog.Infoln("info: init the node cache successfully")
	}
	...
	klog.Infoln("end to wait for cache")
...
func (c *Controller) syncPod(key string) (forget bool, err error) {
	...
	klog.V(2).Infof("begin to sync topology pod %s in ns %s", name, ns)
	if err != nil {
		return false, err
	}
	...
	case errors.IsNotFound(err):
		klog.V(2).Infof("pod %s in ns %s has been deleted.", name, ns)
	...
	case err != nil:
		klog.Warningf("warn: unable to retrieve pod %v from the store: %v", key, err)
	default:
		if utils.IsCompletePod(pod) {
			klog.V(2).Infof("pod %s in ns %s has completed.", name, ns)
			c.schedulerCache.RemovePod(pod)
		} else {
...
func (c *Controller) processNextWorkItem() bool {
	klog.V(2).Infof("begin processNextWorkItem()")
	key, quit := c.podQueue.Get()
	if quit {
		return false
	}
	defer c.podQueue.Done(key)
	defer klog.V(2).Infof("end processNextWorkItem()")
...
func (c *Controller) addPodToCache(obj interface{}) {
	pod, ok := obj.(*v1.Pod)
	if !ok {
		klog.Warningf("Failed to convert to *v1.Pod: %#v", obj)
		return
	}
...
func (c *Controller) updatePodInCache(oldObj, newObj interface{}) {
	...
	if !c.schedulerCache.KnownPod(podUID) && utils.GetGPUIDFromAnnotation(newPod) != "" {
		needUpdate = true
	}
	if needUpdate {
		...
		klog.Infof("Need to update pod name %s in ns %s and old status is %v, new status is %v; its old annotation %v and new annotation %v",
			newPod.Name,
			newPod.Namespace,
			oldPod.Status.Phase,
			newPod.Status.Phase,
		...
		klog.Infof("No need to update pod name %s in ns %s and old status is %v, new status is %v; its old annotation %v and new annotation %v",
			newPod.Name,
			newPod.Namespace,
			oldPod.Status.Phase,
			newPod.Status.Phase,
...
func (c *Controller) deletePodFromCache(obj interface{}) {
	...
	klog.V(2).Infof("delete pod %s in ns %s", pod.Name, pod.Namespace)
	podKey, err := KeyFunc(pod)
	if err != nil {
		klog.Warningf("Failed to get the jobkey: %v", err)
		return
